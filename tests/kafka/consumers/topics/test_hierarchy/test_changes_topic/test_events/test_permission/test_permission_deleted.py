...
async def test_delete_permission_not_existing(
    async_elastic_session: AsyncElasticsearch,
    new_permission_data: ListHierarchyPermission,
):
    kfk_msg = KafkaMSGMock(
        msg_key=f"{HierarchyMessageType.HIERARCHY_PERMISSION.value}:{ObjEventStatus.UPDATED.value}",
...
async def test_delete_permission_existing(
    ...
    # change name
    new_permission_msg_objects = []
    for obj in existing_permission_data.objects:
        obj.permission += "_new"
        new_permission_msg_objects.append(obj)
    new_permission_msg = ListHierarchyPermission(
        objects=new_permission_msg_objects
    )

    kfk_msg = KafkaMSGMock(
        msg_key=f"{HierarchyMessageType.HIERARCHY_PERMISSION.value}:{ObjEventStatus.UPDATED.value}",
        msg_value=new_permission_msg.SerializeToString(),
        msg_topic=KAFKA_HIERARCHY_CHANGES_TOPIC,
    )
    handler = HierarchyChangesTopicHandler(kafka_msg=kfk_msg)
    await handler.process_the_message()
    ...
    for item_data in result["hits"]["hits"]:
        ...
        assert not set(incoming_permissions).difference(permissions)
