...
async def test_update_level_not_existing(
    async_elastic_session: AsyncElasticsearch, new_level_msg: ListLevel
):
    """Ignore if items not exist in level"""
    kfk_msg = KafkaMSGMock(
        msg_key=f"{HierarchyMessageType.LEVEL.value}:{ObjEventStatus.UPDATED.value}",
    ...
    query = {"terms": {"id": [i.id for i in new_level_msg.objects]}}
...
@pytest.mark.asyncio(loop_scope="session")
async def test_update_level_existing(
    ...
    """
    # change name
    new_level_msg_objects = []
    for obj in existing_level_msg.objects:
        obj.name += "_new"
        new_level_msg_objects.append(obj)
    new_level_msg = ListLevel(objects=new_level_msg_objects)
    ...
    # comparison
    query = {"terms": {"id": [i.id for i in existing_level_msg.objects]}}
    result = await async_elastic_session.search(
        query=query, index=HIERARCHY_LEVELS_INDEX
    )
    assert result["hits"]["hits"]

    saved_level = [i["_source"] for i in result["hits"]["hits"]]
    assert len(saved_level) == len(new_level_msg_objects)
    income_level = protobuf_kafka_msg_to_dict(
        msg=new_level_msg,  # noqa
        including_default_value_fields=True,
    )["objects"]
    assert saved_level == income_level
