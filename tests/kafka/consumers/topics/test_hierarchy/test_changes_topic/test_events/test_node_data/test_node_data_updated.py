...
async def test_update_node_data_not_existing(
    async_elastic_session: AsyncElasticsearch, new_node_data_msg: ListNodeData
):
    kfk_msg = KafkaMSGMock(
        msg_key=f"{HierarchyMessageType.NODE_DATA.value}:{ObjEventStatus.UPDATED.value}",
...
@pytest.mark.asyncio(loop_scope="session")
async def test_update_node_data_existing(
