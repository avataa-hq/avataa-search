...
async def test_create_hierarchy_not_existing(
    async_elastic_session: AsyncElasticsearch, new_hierarchy_msg: ListHierarchy
):
    kfk_msg = KafkaMSGMock(
        msg_key=f"{HierarchyMessageType.HIERARCHY.value}:{ObjEventStatus.CREATED.value}",
...
@pytest.mark.asyncio(loop_scope="session")
async def test_create_hierarchy_existing(
