...
STATUSES_DO_NOTHING = ["deleted", "updated", "not_existing_status"]
...
async def test_process_handler_with_create_event_case_1(async_elastic_session):
    """TEST With receiving Process: updated deleted or other statuses msg - if MO does not exist:
    ...
    for status in STATUSES_DO_NOTHING:
...
async def test_with_create_process_case_2(async_elastic_session):
    ...
    data_before = search_res["hits"]["hits"][0]["_source"]
    ...
    for status in STATUSES_DO_NOTHING:
        ...
        data_after = search_res["hits"]["hits"][0]["_source"]
