...
async def with_process_instance_externally_terminated(
    ...
    while retry_count < retries:
        ...
        if existing_mo_data:
            ...
            updated_data[ZeebeProcessInstanceFields.STATE.value] = (
                CamundaOperateStatuses.EXTERNALLY_TERMINATED.value
            )

            try:
                await elastic_client.update(
                    index=index, id=mo_id, doc=updated_data
                )
        ...
        if retry_count < retries:
            print(
                f"Retrying [in externally terminated process] in 1 second... Attempt {retry_count}"
            )
            await asyncio.sleep(1)
        else:
            print(
                "Max retry attempts reached, exiting from externally terminated process."
            )
